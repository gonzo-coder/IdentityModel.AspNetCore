...
{
    /// <summary>
    /// Delegating handler that injects a password access token into an outgoing request
    /// </summary>
    public class PasswordAccessTokenHandler : DelegatingHandler
    {
        ...
        public PasswordAccessTokenHandler(IAccessTokenManagementService accessTokenManagementService, string tokenClientName = AccessTokenManagementDefaults.DefaultTokenClientName)
        {
        ...
        {
            var token = await _accessTokenManagementService.GetPasswordAccessTokenAsync(_tokenClientName, forceRenewal);
