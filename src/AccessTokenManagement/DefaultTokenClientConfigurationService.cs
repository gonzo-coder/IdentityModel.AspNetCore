...
namespace IdentityModel.AspNetCore.AccessTokenManagement
{
  ...
  public class DefaultTokenClientConfigurationService : ITokenClientConfigurationService
  {
    ...
    /// <inheritdoc />
    public virtual async Task<PasswordTokenRequest> GetPasswordGrantRequestAsync(string clientName)
    {
      ...
      {
        // if only one client configuration exists, load that
        if (_accessTokenManagementOptions.Password.Clients.Count == 1)
        {
          requestDetails = _accessTokenManagementOptions.Password.Clients.First().Value;
        }
        // otherwise fall back to the scheme configuration
        else
        {
          var (options, configuration) = await GetOpenIdConnectSettingsAsync(_accessTokenManagementOptions.Password.Scheme);

          requestDetails = new PasswordTokenRequest()
          {
            Address = configuration.TokenEndpoint,
          ...
          if (!string.IsNullOrWhiteSpace(_accessTokenManagementOptions.Password.Scope))
          {
            requestDetails.Scope = _accessTokenManagementOptions.Password.Scope;
          }
      ...
      {
        if (!_accessTokenManagementOptions.Password.Clients.TryGetValue(clientName, out requestDetails))
        {
          throw new InvalidOperationException($"No access token client configuration found for client: {clientName}");
        }
      }
    ...
    /// <inheritdoc />
    public virtual async Task<RefreshTokenRequest> GetRefreshPasswordTokenRequestAsync(string clientName)
    {
      PasswordTokenRequest configuration = await GetPasswordGrantRequestAsync(clientName);
      var requestDetails = new RefreshTokenRequest
      {
