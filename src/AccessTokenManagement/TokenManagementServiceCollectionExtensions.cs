...
namespace Microsoft.Extensions.DependencyInjection
{
  ...
  public static class TokenManagementServiceCollectionExtensions
  {
    ...
    public static TokenManagementBuilder AddAccessTokenManagement(this IServiceCollection services, Action<AccessTokenManagementOptions> options = null)
    {
      ...
      services.AddTransient<PasswordAccessTokenHandler>();
      ...
      services.TryAddTransient<IPasswordAccessTokenCache, PasswordTokenCache>();
    ...
    /// <summary>
    /// Adds a named HTTP client for the factory that automatically sends the a password access token
    /// </summary>
    /// <param name="services"></param>
    /// <param name="clientName">The name of the client.</param>
    /// <param name="tokenClientName">The name of the token client.</param>
    /// <param name="configureClient">Additional configuration.</param>
    /// <returns></returns>
    public static IHttpClientBuilder AddPasswordAccessTokenClient(this IServiceCollection services, string clientName, string tokenClientName = AccessTokenManagementDefaults.DefaultTokenClientName, Action<HttpClient> configureClient = null)
    {
      if (configureClient != null)
      {
        return services.AddHttpClient(clientName, configureClient)
            .AddPasswordAccessTokenHandler(tokenClientName);
      }
    ...
    /// <param name="tokenClientName"></param>
    /// <returns></returns>
    public static IHttpClientBuilder AddPasswordAccessTokenHandler(this IHttpClientBuilder httpClientBuilder, string tokenClientName = AccessTokenManagementDefaults.DefaultTokenClientName)
    {
      return httpClientBuilder.AddHttpMessageHandler(provider =>
      {
        var accessTokenManagementService = provider.GetRequiredService<IAccessTokenManagementService>();

        return new PasswordAccessTokenHandler(accessTokenManagementService, tokenClientName);
      });
    }
  }
}